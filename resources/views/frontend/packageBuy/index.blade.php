@extends('frontend.layouts.frontend_master')

@push('css')
<style>
    .paymentWrap {
        padding: 50px;
    }

    .paymentWrap .paymentBtnGroup {
        max-width: 800px;
        margin: auto;
    }

    .paymentWrap .paymentBtnGroup .paymentMethod {
        padding: 40px;
        box-shadow: none;
        position: relative;
    }

    .paymentWrap .paymentBtnGroup .paymentMethod.active {
        outline: none !important;
    }

    .paymentWrap .paymentBtnGroup .paymentMethod.active .method {
        border-color: #4cd264;
        outline: none !important;
        box-shadow: 0px 3px 22px 0px #7b7b7b;
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method {
        position: absolute;
        right: 3px;
        top: 3px;
        bottom: 3px;
        left: 3px;
        background-size: contain;
        background-position: center;
        background-repeat: no-repeat;
        border: 2px solid transparent;
        transition: all 0.5s;
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method.visa {
        background-image: url("data:image/png;base64,");
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method.master-card {
        background-image: url("data:image/png;base64,");
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method.amex {
        background-image: url("http://www.paymentscardsandmobile.com/wp-content/uploads/2015/08/Amex-icon.jpg");
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method.vishwa {
        background-image: url("http://i.imgur.com/VkiM7PL.jpg");
    }

    .paymentWrap .paymentBtnGroup .paymentMethod .method.ez-cash {
        background-image: url("http://www.busbooking.lk/img/carousel/BusBooking.lk_ezCash_offer.png");
    }


    .paymentWrap .paymentBtnGroup .paymentMethod .method:hover {
        border-color: #4cd264;
        outline: none !important;
    }
</style>
@endpush

@section('content')
<div class="container">
    <form action="{{ route('package.store') }}" method="post">
        @csrf
        {{-- ------------Product Details and checkout summary Section start---- --}}
        <div class="row">
            <div class="col-md-12 mt-5">
                <div class="m-auto justify-content-center d-flex">
                    <button class="btn btn-outline-info col-md-8" type="button" data-toggle="collapse" data-target="#collapseExample1" aria-expanded="false" aria-controls="collapseExample">
                        Product Details & Checkout Summary
                    </button>
                </div>
                <br>


                <div class="collapse show" id="collapseExample1">
                    <div class="card card-body">
                        <div class="col-md-12 m-auto">
                            <div class="card mt-2">
                                <div class="card-header">
                                    <h5>Product Details & Checkout Summary</h5>
                                </div>
                                <div class="card-body table-responsive">
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr class="table-active">
                                                <th class="text-center">Name</th>
                                                <th class="text-center">Photo</th>
                                                <th class="text-center">original Price</th>
                                                <th class="text-center">Package Discount</th>
                                                <th class="text-center">Tax</th>
                                                <th class="text-center">Package Price</th>
                                                <th class="text-center" width="100">Quantity</th>
                                                <th class="text-center">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td class="text-center">{{ $packages->relationToProductTable->name }}</td>
                                                <td class="text-center">
                                                    <img src="{{ asset('uploads/product_photos') }}/{{ $packages->relationToProductTable->photo }}" height="70 px" width="70 px" alt="product photo">
                                                </td>
                                                <td class="text-center">{{ $packages->relationToProductTable->unit_price }}</td>
                                                <td class="text-center">{{ $packages->relationToDiscountRulesTable->percentage }}%</td>
                                                <td class="text-center">{{ $packages->tax_percentage }}%</td>
                                                <td class="text-center">{{ $packages->price }} <br> [With Tax]</td>
                                                <td>
                                                    <input type="text" name="quantity" value="{{ Session::get('quantity') ?? '1'}}" id="quantity" class="form-control">
                                                </td>

                                                <td class="text-center">
                                                    <div class="btn" role="group" aria-label="Basic example">
                                                        {{-- <a href="" class="btn btn-sm btn-outline-secondary mr-2">Update</a>--}}
                                                        {{-- |--}}
                                                        <a href="{{ '/' }}" class="text-danger"><i class="fa fa-trash fa-2x ml-2"></i></a>
                                                    </div>
                                                </td>

                                            </tr>
                                        </tbody>
                                    </table>

                                    {{-- Coupon section start--}}
                                    <div class="container">
                                        <div class="row">
                                            <div class="col-md-6"></div>
                                            <div class="col-md-6 mb-5 mt-5">
                                                <div class="row">
                                                    <div class="col-md-8">
                                                        <div class="md-form">
                                                            <label class="">Coupon Code</label>
                                                            <input type="text" name="coupon_code" value="{{ $coupon_code }}" id="coupon_code" class="form-control">
                                                            <input type="hidden" name="coupon_discount_amount" value="{{ $coupon_discount_amount }}" class="form-control">
                                                            <input type="hidden" name="product_id" value="{{ $packages->product_id }}" class="form-control">
                                                            <input type="hidden" name="package_id" value="{{ $packages->id }}" class="form-control">
                                                            <input type="hidden" name="unit_price" value="{{ $packages->relationToProductTable->unit_price }}" class="form-control">
                                                            <input type="hidden" name="tax_percentage" value="{{ $packages->tax_percentage }}" class="form-control">
                                                            <input type="hidden" name="cash_discount" value="{{ $packages->price }}" class="form-control">
                                                            <input type="hidden" name="quantity" value="{{ Session::get('quantity') ?? '1' }}" class="form-control">

                                                            <br>
                                                            @if($errors->all())
                                                                @foreach($errors->all() as $error)
                                                                    <span class="text-danger">{{ $error }}</span>
                                                                @endforeach
                                                            @endif
                                                        </div>
                                                    </div>
                                                    <div class="mt-3">
                                                        <a type="button" id="apply_code_btn" style="margin-top: 15px;" class="btn btn-secondary text-white">Apply Code</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    {{-- Coupon section end--}}
                                    <div class="container">
                                        <div class="row">
                                            <div class="col-md-8"></div>
                                            <div class="col-md-4 mt-4">
                                                <table class="table table-borderless">
                                                    <tbody>
                                                        <tr>
                                                            <th class="text-center">Sub Total</th>
                                                            @if(Session::get('quantity'))
                                                            <td class="text-center" id="sub_total">{{ $packages->price * Session::get('quantity') }} Tk.</td>
                                                            @else
                                                            <td class="text-center" id="sub_total">{{ $packages->price }} Tk.</td>
                                                            @endif
                                                        </tr>
                                                        <tr>
                                                            <th class="text-center">Shipping(+)</th>
                                                            <td class="text-center" id="shipping_charge">50 TK.</td>
                                                            <input type="hidden" name="shipping_charge" value="50">
                                                        </tr>
                                                        <tr>
                                                            <th class="text-center">Coupon Discount(-)</th>
                                                            @if(Str::endsWith($coupon_discount_amount, '%'))
                                                            <td class="text-center">{{ $coupon_discount_amount }}</td>
                                                            @else
                                                            <td class="text-center">{{ $coupon_discount_amount }} TK.</td>
                                                            @endif
                                                        </tr>

                                                        <tr>
                                                            <th class="text-center">Total</th>
                                                            @if(Str::endsWith($coupon_discount_amount, '%'))
                                                            <td class="text-center" id="total">
                                                                {{ ($packages->price+50) - (((Str::before($coupon_discount_amount, '%')) * ($packages->price+50)) / 100) }} TK.
                                                            </td>
                                                            @else
                                                            <td class="text-center" id="total">{{ $packages->price * Session::get('quantity') + 50-$coupon_discount_amount }} TK.</td>
                                                            @endif
                                                        </tr>

                                                        <tr class="table-danger">
                                                            <th class="text-center">Payable Total</th>
                                                            @if(Str::endsWith($coupon_discount_amount, '%'))
                                                            <td class="text-center" id="total">
                                                                {{ ($packages->price+50) - (((Str::before($coupon_discount_amount, '%')) * ($packages->price+50)) / 100) }} TK.
                                                            </td>
                                                            @else
                                                            <td class="text-center" id="total">{{ $packages->price+50-$coupon_discount_amount }} TK.</td>
                                                            @endif
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        {{-- ------------Product Details and checkout summary Section end------ --}}

        {{-- Shipping Address start--}}
        <div class="row">
            <div class="col-md-12 mt-lg-5">
                <div class="m-auto justify-content-center d-flex">
                    <button class="btn btn-outline-info col-md-8" type="button" data-toggle="collapse" data-target="#collapseExample2" aria-expanded="false" aria-controls="collapseExample">
                        Shipping Address
                    </button>
                </div>
                <br>
                <div class="collapse" id="collapseExample2">
                    <div class="card card-body">
                        <div class="row">
                            <div class="col-md-8 m-auto">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="md-form">
                                            <input type="text" value="{{ Auth()->user()->name ?? '' }}" name="name" class="form-control">
                                            <label class=""><b>Your name</b></label>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="md-form">
                                            <input type="email" value="{{ Auth()->user()->email ?? '' }}" name="email" class="form-control">
                                            <label class=""><b>Your email</b></label>
                                        </div>
                                    </div>
                                </div>
                                <br>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="md-form">
                                            <input type="text" value="{{ Auth()->user()->phone ?? '' }}" name="phone" class="form-control">
                                            <label class=""><b>Phone No</b></label>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="md-form">
                                            <input type="text" id="alt_phone_no" value="{{ Auth()->user()->relationToProfile->alternate_phone ?? '' }}" name="alternate_phone" class="form-control">
                                            <label class=""><b>Alternative Phone No</b></label>
                                        </div>
                                    </div>
                                </div>
                                <br>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="md-form">
                                            <textarea type="text" name="address" rows="2" class="form-control md-textarea">{{ Auth()->user()->relationToProfile->address ?? '' }}</textarea>
                                            <label><b>Address</b></label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        {{-- Shipping Address end--}}

        {{-- Continue to payment section start--}}
        <div class="row">
            <div class="col-md-12 mt-lg-5">
                <div class="m-auto justify-content-center d-flex">
                    <button class="btn btn-outline-info col-md-8" type="button" data-toggle="collapse" data-target="#collapseExample3" aria-expanded="false" aria-controls="collapseExample">
                        Continue To Payment
                    </button>
                </div>
                <br>
                <div class="collapse" id="collapseExample3">
                    <div class="container">
                        <div class="row">
                            <div class="paymentCont m-auto">
                                <div class="headingWrap">
                                    <h3 class="headingTop text-center">Select Your Payment Method</h3>
                                </div>
                                <div class="paymentWrap">
                                    <div class="btn-group paymentBtnGroup btn-group-justified" data-toggle="buttons">
                                        <label class="btn paymentMethod active">
                                            <div class="method visa"></div>
                                            <input type="radio" value="bkash" name="payment_method" checked>
                                        </label>
                                        <label class="btn paymentMethod">
                                            <div class="method master-card"></div>
                                            <input type="radio" value="master-card" name="payment_method">
                                        </label>
                                        <label class="btn paymentMethod">
                                            <div class="method amex"></div>
                                            <input type="radio" value="hand_cash" name="payment_method">
                                        </label>
                                    </div>
                                </div>

                                <div class="footerNavWrap clearfix">
                                    <button type="submit" class="btn btn-success pull-right btn-fyi mb-4">CONFIRM ORDER<span class="glyphicon glyphicon-chevron-right"></span></button>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        {{-- Continue to payment section end--}}
    </form>
</div>
@endsection

@section('footer_scripts')
<script type="text/javascript">
    $(document).ready(function() {

        $("#apply_code_btn").click(function() {
            var coupon_code = $("#coupon_code").val();

            var link_to_go = window.location.href + "/" + coupon_code;
            window.location.href = link_to_go;

            let searchParams = new URLSearchParams(window.location.search)
        });


        $("#quantity").change(function() {
            var qty = $("#quantity").val();
            var go = "{{ route('calculateQty') }}" + "/" + qty;
            window.location.href = go;
        });


    });
</script>
@endsection
